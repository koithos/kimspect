...
async fn test_get_pod_images() -> Result<()> {
    ...
    let result = client
        .get_pod_images("default", None, None, None, None, false)
        .await;
...
async fn test_get_pod_images_all_namespaces() -> Result<()> {
    ...
    let _images = client
        .get_pod_images("default", None, None, None, None, true)
        .await?;
...
async fn test_get_pod_images_with_node_and_all_namespaces() -> Result<()> {
    ...
    let result = client
        .get_pod_images("default", Some("non-existent-node"), None, None, None, true)
        .await;
    assert!(matches!(result, Err(e) if e.downcast_ref::<K8sError>().is_some()));
    Ok(())
}
...
async fn test_get_pod_images_with_pod_and_all_namespaces() -> Result<()> {
    ...
    let result = client
        .get_pod_images("default", None, Some("non-existent-pod"), None, None, true)
        .await;
    assert!(matches!(result, Err(e) if e.downcast_ref::<K8sError>().is_some()));
    Ok(())
}
